import math

op = [sum, math.prod, min, max,
      lambda ls: ls[0],  # literal
      lambda ls: 1 if ls[0] > ls[1] else 0,  # gt
      lambda ls: 1 if ls[0] < ls[1] else 0,  # lt
      lambda ls: 1 if ls[0] == ls[1] else 0]  # eq


def read_input(filename):
    with open(filename, "r") as f:
        return f.read().strip()


def solution(bit_string, startbit):
    # Solution by 'ai_prof'
    # https://topaz.github.io/paste/#

    index = startbit
    packet_type_id = int(bit_string[index+3:index+6], 2)
    index += 6
    values = []

    # If literal value
    if packet_type_id == 4:
        values = [0]
        while True:
            values[0] = 16 * values[0] + int(bit_string[index+1:index+5], 2)
            index += 5
            # Last packet
            if bit_string[index-5] == '0':
                break
    elif bit_string[index] == '0':
        end_index = index + 16 + int(bit_string[index+1:index+16], 2)
        index += 16
        while index < end_index:
            index, value = solution(bit_string, index)
            values.append(value)
    else:
        np = int(bit_string[index+1:index+12], 2)
        index += 12
        for _ in range(np):
            index, v = solution(bit_string, index)
            values.append(v)
    return index, op[packet_type_id](values)


def main():
    hex_msg = read_input("input.txt")
    bits = "".join(format(int(char, 16), '04b') for char in hex_msg)
    print("Total value:", solution(bits, 0)[1])


if __name__ == '__main__':
    main()
